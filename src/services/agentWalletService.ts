...
class AgentWalletService {
  ...
  generateAgentAddress(userAddress: string): string {
    // DEPLOYED SMART CONTRACT on BNB Testnet
    // This is a real AgentWallet contract that can receive BNB and execute actions
  ...
  /**
   * Transfer BNB to agent wallet (REAL transaction)
   */
  async transferBNBToAgent(
    fromAddress: string,
    agentAddress: string,
    amountBNB: number,
    privyClient: any
    ...
    try {
      ...
      console.log('Sending BNB to agent:', {
        from: fromAddress,
        to: agentAddress,
        amount: amountBNB,
        tx
  ...
  /**
   * Get real transaction details from BNB testnet
   */
