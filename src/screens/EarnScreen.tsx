...
export const EarnScreen = ({ sendTransaction, wallets }: EarnScreenProps = {}) => {
  ...
  const pools = [
    { name: 'BNB-USDT Pool', apy: 12.5, tvl: '2.5M', risk: 'Medium', address: '0xF9c36b4fBA23F515b1ae844642F81DC0aDdf6AF6' },
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      if (txHash) {
        ...
        Alert.alert(
          '✅ Deposit Successful!',
          `Added ${depositAmount} BNB to ${selectedPool.name}\n\nTx: ${txHash.substring(0, 10)}...`,
          [
            { text: 'Close', style: 'cancel' },
            { text: 'View on BNBScan', onPress: () => Linking.openURL(`https://testnet.bscscan.com/tx/${txHash}`) }
          ]
        );
      }
    } catch (error: any) {
      console.error('❌ Deposit error:', error);
      Alert.alert('Error', error.message || 'Failed to deposit BNB');
    } finally {
      setDepositing(false);
    }
  };
            ...
            {totalDeposited > 0 && (
              <View style={styles.depositedInfo}>
                ...
                <Text style={styles.depositedText}>
                  Total Deposited: {totalDeposited.toFixed(4)} BNB
                </Text>
              </View>
            )}
