...
async function main() {
  ...
  const spendingCap = hre.ethers.utils.parseEther("1"); // 1 BNB spending cap

  console.log("Deployment parameters:");
  console.log("- Owner:", ownerAddress);
  console.log("- Spending Cap:", hre.ethers.utils.formatEther(spendingCap), "BNB\n");
  ...
  console.log("🔗 View on BNBScan:", `https://testnet.bscscan.com/address/${agentWallet.address}\n`);

  // Send initial BNB to contract
  ...
  console.log("📊 Contract balance:", hre.ethers.utils.formatEther(balance), "BNB");

  // Get contract stats
  const stats = await agentWallet.getStats();
  console.log("\n📈 Agent Stats:");
  console.log("- Balance:", hre.ethers.utils.formatEther(stats.balance), "BNB");
  console.log("- Credit Allocated:", stats._creditAllocated.toString());
  console.log("- Credit Used:", stats._creditUsed.toString());
  console.log("- Spending Cap:", hre.ethers.utils.formatEther(stats._spendingCap), "BNB");
